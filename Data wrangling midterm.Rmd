...
Installing packages to import the .XPT file and then converting it into .csv file
```{r}
...
I am dropping some of the coloumns as they are creating a lots of noises and is not adding much to the data. For example the coloumn DIQ010 represents if the person has diabetes or not, which is not adding something new as this can also be interpreted from the coloumn reprsenting age when diabetes was first recognized. This can be also shown by counting the number of counts for 'yes' in DIQ010 which is approximately equal to cumulative of DID040(AGE) excluding the dont know, refused and missing responses. Same with DIQ240(past year how many times seen a doctor) which doesnt add any new information as this information can be taken out from the coloumn DID250(past year how many times seen a doctor). This can also be shown by noticing that 'yes' count is 643 which exactly equal to the cumulative responses for DID250 excluding the missing values.
```{r}
#Dropping columns
mt1$DIQ010<-NULL
mt1$DIQ050<-NULL
...
Just renaming columns to make it more readable and interpretable
```{r}
...
```
Its easier to change into factors to perform other functions or commands or play with data as it becomes more flexible
```{r}
str(mt1)
...
```
with str(mt1), we can check that data type has been actually changes
```{r}
str(mt1)
```
...
changing column names for the reasons to make it simpler for reading and also for doing further code for analysis
```{r}
...
Here we are first converting some coloums to numeric to make the null values zero 
Then we are using ifelse command to multiply values where value is "1" and "2" in units. We are basically trying to normalize the data by putting all the numeric values in one coloumn and another coloumns will represent the unit corresponding to the first coloumn.
```{r}
...
```
Normalizing the data by showing all the units to "month"
```{r}
mt1$I_UNIT <- factor(mt1$I_UNIT, levels = c(1,2), labels = c("month","month"))
```
Then we are using ifelse command to multiply values for year, month, week and day where value is "1","2","3" and "4" in units. We are basically trying to normalize the data by putting all the numeric values in one coloumn  and changing it into per years from per day, per week and per month and another coloumns will represent the unit corresponding to this first coloumn.
```{r}
...
```
Normalizing the data by showing all the units to "per year"
```{r}
...
Another thing we can do is calculate the desired changes in each of the SBP level, DBP level and LDL level.
The desired changes can be negative or positive. 

But it should be kept in mind that coloumn first will have to converted into numeric datatype to impute the NA values with 0 and so that it won't throw an error while running the command
```{r}
...
```
Calculating the desired changes in SBP level. 
```{r}
mt1$desired_SBP <- (mt1$SBP.SHOULD.BE. - mt1$SBP.RECENT.)
```
Calculating the desired changes in DBP level. 
```{r}
mt1$desired_DBP <- (mt1$DBP.SHOULD.BE. - mt1$DBP.RECENT.)
```
Calculating the desired changes in LDL level. 
```{r}
mt1$desired_LDL <- (mt1$LDL.SHOULD.BE. - mt1$LDL.RECENT.)
```
We mainly did the above code to get the gist of the data that is present in those coloums and what it says and how it can be converted into another coloumn so that after creating these new columns with desired changes we can drop/delete the original coloumns. However I am not dropping them because it might be helpul to others if they are doing the analysis on their own and if they dont want the original coloumns, they can drop them.
```{r}
...
##mean is 48.48692 
```
I didnt impute the missing values with the mean as I believe that I would lose some useful data while doing so.
